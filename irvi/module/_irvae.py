...
class TCREncoder(nn.Module):
    ...
    def __init__(
        ...
        # Pooling strategy
        self.pooling_strategy = "attention_weighted"  # Options: "mean", "max", "attention_weighted"
    ...
    def forward(self, inputs, attention_mask=None):
        ...
        embeddings = self.embedding(token_ids) * torch.sqrt(torch.tensor(self.d_model, dtype=torch.float32, device=token_ids.device))
    ...
    def _pool_sequence(self, hidden_states: torch.Tensor, attention_mask: torch.Tensor) -> torch.Tensor:
        ...
        if self.pooling_strategy == "mean":
            ...
            attention_weights = self.attention_pooling(hidden_states).squeeze(-1)  # (batch_size, seq_len)
            ...
            # Weighted sum
            weighted_repr = torch.bmm(attention_weights.unsqueeze(1), hidden_states).squeeze(1)  # (batch_size, d_model)
...
class IRVAE(BaseMinifiedModeModuleClass):
    ...
    def __init__(
        ...
        if gene_likelihood in ["zinb", "nb"]:
            self.px_scale_decoder = nn.Sequential(
                nn.Linear(n_hidden, n_input_genes),
                nn.Softmax(dim=-1)
            )
        ...
        self.tcr_decoder = nn.Sequential(
            ...
            nn.Linear(tcr_d_model, actual_vocab_size * actual_max_length)  # Use actual dimensions
        )
    ...
    def generative(
        ...
        batch_size = z.shape[0] if z.dim() == 2 else z.shape[1]  # Handle different z shapes
    ...
    def loss(
        ...
        return LossOutput(
            ...
            extra_metrics={
                ...
                "total_reconstruction_loss": (reconst_loss_gene + weighted_tcr_loss).mean(),
            }
        )
