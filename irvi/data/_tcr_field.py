...
class TCRField(BaseAnnDataField):
    ...
    def validate_field(self, adata: AnnData) -> None:
        ...
        if self._original_attr_key in adata.obs.columns:
            ...
            raise ValueError(f"TCR data not found in adata.obs['{self._original_attr_key}'] or adata.obsm['{self._original_attr_key}']")
        ...
        for i, seq in enumerate(sequences):
            ...
            if invalid_chars:
                print(f"Warning: Sequence {i} contains invalid amino acids: {invalid_chars}")
    ...
    def register_field(self, adata: AnnData) -> dict:
        ...
        if isinstance(sequences, list) and len(sequences) > 0 and isinstance(sequences[0], str):
            tokenized_sequences = []
            for seq in sequences:
                if pd.isna(seq) or seq == '':
                    ...
                    tokenized_seq = self.tokenizer.encode(str(seq)).numpy().astype(np.int64)
