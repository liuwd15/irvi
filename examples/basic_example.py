...
def create_synthetic_data(n_cells=1000, n_genes=500):
    ...
    for cell_type in range(n_cell_types):
        ...
        base_expression = np.random.negative_binomial(
            5, 0.3, size=(cells_per_type, n_genes)
        )
    ...
    if remaining > 0:
        ...
        extra_X = np.random.negative_binomial(5, 0.3, size=(remaining, n_genes)).astype(
            np.float32
        )
...
def main():
    ...
    for i, cell_type in enumerate(adata.obs["cell_type"].unique()):
        ...
        axes[0].scatter(
            latent[mask, 0], latent[mask, 1], label=cell_type, alpha=0.7, s=20
        )
    ...
    for i, cell_type in enumerate(adata.obs["cell_type"].unique()):
        mask = adata.obs["cell_type"] == cell_type
        axes[1].scatter(
            gene_embedding[mask, 0],
            gene_embedding[mask, 1],
            label=cell_type,
            alpha=0.7,
            s=20,
        )
    ...
    for i, cell_type in enumerate(adata.obs["cell_type"].unique()):
        mask = adata.obs["cell_type"] == cell_type
        axes[2].scatter(
            tcr_embedding[mask, 0],
            tcr_embedding[mask, 1],
            label=cell_type,
            alpha=0.7,
            s=20,
        )
